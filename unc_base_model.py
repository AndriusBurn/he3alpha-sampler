...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct  3 19:58:31 EDT 2023
...
class BaseModel:
    def __init__(self, data, norm_grouping, param_prior, norm_prior, use_theory_cov):
        ...
        # Set hyperparameters (used for updating prior on unc parameters)
        self.N_obs = self.cs_data.shape[0]
        self.n_c = 2
        self.nu_0 = 1.5
        self.nu = self.nu_0 + self.N_obs * self.n_c
        self.tau = 1.5
        ...
        # self.cs_LO_values = defined in model subclass
        ...
        self.total_dim = self.bounds.shape[0]
        ...
        Q_Lambda = ((np.array([max(q[t], self.kvalue_cs[t])
                    for t in range(len(theta_rad))])))
        self.Q_Lambda = np.reshape(Q_Lambda, (1, len(Q_Lambda)))
    ...
    def cov_theory(self, c_bar_squared, Lambda_B):
        """
        Computes the covariance theory matrix given c_bar_squared and Lambda_B.
        """
        K = 2.0  # N3LO is taken as dominant theoretical error
        Q = self.Q_Lambda / Lambda_B
        cov_theory_cs = (c_bar_squared * self.y_ref * (np.power(Q,(K + 1)))).transpose() @ (
            c_bar_squared * self.y_ref * (np.power(Q, K + 1)))  #/(1 - Q.transpose() @ Q)
    ...
    def log_c_bar_squared_post(self, c_bar_squared):
        """
        Computes the log of the c_bar_squared posterior P(c_bar_squared | Lambda_B, theta, I) which is 
        distributed according to the inverse chi^2 distribution.
        """
        inv_chi_squared = (((self.nu * self.tau**2)**(self.nu / 2)) / (gamma(self.nu / 2) * (c_bar_squared**(1 + self.nu / 2))) * 
                                np.exp(-self.nu * self.tau**2 / (2 * c_bar_squared)))
        return np.log(inv_chi_squared)



    def log_Lambda_B_prior(self, Lambda_B):
        sigma_B = 1.0 # fm
        Lambda_B_0 = 1.01354604286 # fm 
        # Prior P(Lambda_B | I) is a gaussian centered at 200 MeV / h_bar, with width 1.0 fm
        gauss_prior = (1 / (np.sqrt(2 * np.pi) * sigma_B)) * np.exp(-(Lambda_B - Lambda_B_0)**2 / (2 * sigma_B**2))

        # Now we need to get the un-normalized posterior of Lambda_B
    ...
    def log_likelihood(self, parameters, theory):
        """
    ...
    def log_posterior(self, parameters):
        """
        Returns the log posterior given a set of parameters.
        """
        # Separate the unc parameters from the rest of the parameters
        c_bar_squared, Lambda_B = parameters[:2]
        model_parameters = parameters[2:]

        # Perform the model evaluation for orders
        self.y_1 = self.cs_theory(model_parameters[:self.erp_dim], 1) # NLO calculation
        self.y_2 = self.cs_theory(model_parameters[:self.erp_dim], 2) # N2LO calculation

        # Compute the c_ni's
        c_1s = (self.y_1 - self.y_ref) / (self.y_ref * (self.Q_Lambda / Lambda_B))
        c_2s = (self.y_2 - self.y_1) / (self.y_ref * np.square(self.Q_Lambda / Lambda_B))

        # Update the hyperparameter tau
        self.tau += np.sqrt(1 / self.nu * (self.nu_0 * self.tau_0**2 + np.sum(c_1s + c_2s)))

        # Generate the new covariance matrix
        # Set up the covariance matrix
        cov_expt_matrix = np.diag(self.err_cs**2)
        cov_theory_matrix = self.cov_theory(c_bar_squared, Lambda_B)
        ...
        # Get the c_bar_squared prior P(c_bar_squared | Lambda_B, theta, I)
        c_bar_squared_log_P = self.log_c_bar_squared_post(c_bar_squared)

        # Get the log prior on the model parameters
        model_log_P = self.log_prior(model_parameters)

        # Get the Lambda_B posterior P(Lambda_B | theta, I)
        Lambda_B_log_P = self.log_Lambda_B_prior()

        # Compute the log likelihood
        log_L = self.log_likelihood(model_parameters)
